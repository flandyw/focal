...
export interface InboxProjectSuggestion {
  projectId: string
  reason: string
}

export function recentDownloadFingerprint(download: { path: string; size: number; modifiedAt: number }): string {
  return `${download.path}\u0000${download.size}\u0000${download.modifiedAt}`
}
...
export function getInboxProjectSuggestion(
  value: string,
  projects: Project[],
  subjects: Subject[],
): InboxProjectSuggestion | undefined {
  const input = new Set(tokens(value))
  let best: { id: string; score: number; matchedTerm: string } | undefined
  for (const project of projects) {
    ...
    const matchedTerm = [...new Set(terms.filter((term) => input.has(term)))]
      .sort((a, b) => b.length - a.length || a.localeCompare(b))[0]
    if (matchedTerm && score > 0 && (!best || score > best.score)) {
      best = { id: project.id, score, matchedTerm }
    }
  }
  return best
    ? { projectId: best.id, reason: `Matched “${best.matchedTerm}” in the filename` }
    : undefined
}

export function suggestInboxProject(value: string, projects: Project[], subjects: Subject[]): string | undefined {
  return getInboxProjectSuggestion(value, projects, subjects)?.projectId
}
